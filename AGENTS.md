...
# muscle_chair Project Memory (Project-Specific)

## Repository Purpose
- Hackathon project "Macho/Muscle Chair" codebase.
- This repository contains:
  - Central server (backend)
  - Device-side I/O controllers (Raspberry Pi, GPIO/I2C, speaker/motor/LED)
  - Web UI
...
- Keep environment-specific secrets out of Git:
  - never commit `.env`, tokens, API keys, Wi-Fi passwords, IP addresses tied to a private network.
...
- Always document assumptions at the top of unit files (deploy path, user, required permissions).
- Prefer running as a non-root user if possible; only use root when hardware access truly requires it.
